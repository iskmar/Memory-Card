    ...
    if (flippedCards.length === 0) {
      ...
      if (flippedCards[0] === flippedCards[1]) {
        ...
        if (openedCards === imgArray.length) {
          ...
          setTimeout(() => {
            let answer = confirm("Another round?");
            if (answer) {
              //   location.reload();
              resetBoard();
              setTimeout(() => {
